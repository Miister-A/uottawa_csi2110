...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize) {
		...
		for (int i=interval ; i<maxSize+interval ; i=i+interval) {

			long tempWorstCase = 0;
			
			for (int j=0 ; j<10 ; j++) {

				int[] generatedArray = genArray(i); //Generates random array of size i (and i+interval afterwards)
				int[] randomizedGeneratedArray = randomizeArray(generatedArray); //Randomizes the array to make sure it's not sorted

				long startTime = System.nanoTime(); //Starting timer here to measure sorting performance
				Arrays.sort(randomizedGeneratedArray); //Sorting being done
				long endTime = System.nanoTime(); //Ending timer here to measure sorting performance
				long timeTaken = endTime-startTime;

				if (timeTaken>tempWorstCase) {
					tempWorstCase = timeTaken;
				}
			}
			System.out.println("N:"+i+",T(n): "+tempWorstCase+" nanoseconds"+",T(n)/n*n): "+tempWorstCase/(double)(i*i)+",T(n/n*log(n)): "+(double)tempWorstCase/((double)i*(Math.log(i)/Math.log(2))));
		}
	}
	...
	public static long unique1Runtime(int n) {
		int[] generatedArray = genArray(n); //Generates array of size n (entered input parameter)
		int[] randomizedGeneratedArray = randomizeArray(generatedArray); //Randomizes the array to make sure it's not sorted

		long startTime = System.nanoTime(); //Starting timer here to measure Unique1 performance
		Unique1.unique1(randomizedGeneratedArray);
		long endTime = System.nanoTime(); //Ending timer here to measure Unique1 performance
		long timeTaken = endTime-startTime;
	...
	public static long unique2Runtime(int n){
		...
		long startTime = System.nanoTime(); //Starting timer here to measure Unique1 performance
		Unique2.unique2(randomizedGeneratedArray);
		long endTime = System.nanoTime(); //Ending timer here to measure Unique1 performance
		long timeTaken = endTime-startTime;
